...
public class ExpenseResource {
    ...
    public Expense create(final Expense expense) {
        if (expense.amount.compareTo(maxSingleExpenseAmount) > 0) {
            throw new WebApplicationException(
                String.format("Expense amount is larger than the limit (%s)", maxSingleExpenseAmount),
	...
	var endDate = LocalDateTime.now().with(temporalAdjuster);
        ...
        if (currentTotalAmount.add(expense.amount).compareTo(maxMonthlyExpenseAmount) > 0) {
            throw new WebApplicationException(
                String.format("If expense is accepted, it would exceed the monthly expense limit (%s)", maxMonthlyExpenseAmount),
                Response.Status.NOT_ACCEPTABLE);
        }
