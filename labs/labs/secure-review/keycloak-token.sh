...
KEYCLOAK_TOKEN_URL=http://$HOSTNAME/auth/realms/$REALM_NAME/protocol/openid-connect/token
KEYCLOAK_URL=http://$HOSTNAME/auth/realms/$REALM_NAME/protocol/openid-connect/token/introspect

echo 1>&2 "Using Keycloak: $KEYCLOAK_TOKEN_URL"
...
if [ "$TOKEN" == "null" ]; then
    ...
    echo 1>&2 "Token succesfuly retrieved. Showing token roles:"
    ...
    -d "client_id=$CLIENT_ID" \
    -d "client_secret=$CLIENT_SECRET" \
    -d "token=$TOKEN" | jq -r '.realm_access' 1>&2
